...
def get_dfs_with_intersecting_sample_ids(dfs: List[pd.DataFrame]) -> List[str]:
    ...
    for df in dfs:
        if intersecting_sample_ids is None:
            ...
            intersecting_sample_ids = intersecting_sample_ids.intersection(set(df["sample_id"].tolist()))

    for index in range(len(dfs)):
        dfs[index] = dfs[index][dfs[index]["sample_id"].isin(intersecting_sample_ids)].sort_values(by="sample_id")
...
def get_dfs_with_intersecting_columns(dfs: List[pd.DataFrame]) -> List[str]:
    ...
    for df in dfs:
        if intersecting_columns is None:
            ...
            intersecting_columns = intersecting_columns.intersection(set(df.drop(columns=["sample_id"]).columns.tolist()))

    for index in range(len(dfs)):
        dfs[index] = dfs[index][["sample_id"] + sorted(list(intersecting_columns))]
