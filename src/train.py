    ...
    if cfg["model"] == "dl_linear_zero_diagonal":
        if cfg["per_chromosome"]:
            for submodel in model.models:
                current_regularization_loss += regularization_nondiagonal_coeff * (regularization_operation(submodel.upper_triangle_weights).sum() + regularization_operation(submodel.lower_triangle_weights).sum())
        else:
