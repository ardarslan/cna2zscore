    ...
    def _process_dataset(self) -> None:
        ...
        for current_input_data_type in self.input_data_types:
            ...
            if current_input_data_type in ["unthresholded_cna", "thresholded_cna", "rppa"]:
                ...
                elif self.cfg["gene_type"][:len("breast_cancer_scc_genes")] == "breast_cancer_scc_genes":
                    component_ids = self.cfg["gene_type"][len("breast_cancer_scc_genes") + 1:].split("_")
                    component_entrezgene_ids = set()
                    for component_id in component_ids:
                        component_entrezgene_ids = component_entrezgene_ids.union([str(entrezgene_id) for entrezgene_id in pd.read_csv(os.path.join(self.processed_data_dir, f"breast_cancer_scc_genes_component_{component_id.zfill(2)}.tsv"), sep="\t")["gene_id"].tolist()])
