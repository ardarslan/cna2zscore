...
from utils import (get_argument_parser, set_seeds, set_experiment_name, \
                   ...
                   set_hyperparameters_according_to_memory_limits)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, cfg["num_epochs"] + 1):
        current_train_loss_dict = train(cfg=cfg, data_loaders=data_loaders, model=model, loss_function=train_loss_function, dataset=dataset, optimizer=optimizer)
        current_val_loss_dict = validate(cfg=cfg, data_loaders=data_loaders, model=model, loss_function=val_test_loss_function, dataset=dataset, epoch=epoch, logger=logger, summary_writer=summary_writer)
