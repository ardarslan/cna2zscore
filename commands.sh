...
for CANCER_TYPE in 'all'; do
    for DATASET in 'unthresholdedcnapurity2gex' 'rppa2gex'; do
        if [[ $DATASET = 'rppa2gex' ]]; then
            declare -a MODEL_OPTIONS=("mlp" "transformer")
        elif [[ $DATASET = 'unthresholdedcnapurity2gex' ]]; then
            declare -a MODEL_OPTIONS=("mlp" "mlp_per_chromosome_24" "transformer")
        else
        ...
        for MODEL in "${MODEL_OPTIONS[@]}"; do
            ...
            if [[ $MODEL = 'linear' || $MODEL = 'linear_per_chromosome_all' || $MODEL = 'linear_per_chromosome_24' ]]; then
                ...
                L1_REG_DIAGONAL_COEFF_OPTIONS=(0.000001 0.00001 0.0001 0.001)
                L2_REG_DIAGONAL_COEFF_OPTIONS=(0.000001 0.00001 0.0001 0.001)
                L1_REG_NONDIAGONAL_COEFF_OPTIONS=(0.000001 0.00001 0.0001 0.001)
                L2_REG_NONDIAGONAL_COEFF_OPTIONS=(0.000001 0.00001 0.0001 0.001)
                ...
                L1_REG_COEFF_OPTIONS=(0.000001 0.00001 0.0001 0.001)
                L2_REG_COEFF_OPTIONS=(0.000001 0.00001 0.0001 0.001)
                GENE_EMBEDDING_SIZE_OPTIONS=(0)
                NUM_ATTENTION_HEADS_OPTIONS=(0)
